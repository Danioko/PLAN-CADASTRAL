...
(function () {
    ...
    function geographicWindow(center, groundW, groundH) {
        ...
        return L.latLngBounds([center.lat-halfLatDeg,center.lng-halfLngDeg],[center.lat+halfLatDeg,center.lng+halfLngDeg]);
    }
    ...
    function latLngToPixel(latlng, center, groundW, groundH, canvasW, canvasH) {
        ...
        return {x:canvasW/2+(R*Math.cos(lat0)*dLng/groundW)*canvasW,y:canvasH/2-(R*dLat/groundH)*canvasH};
    }
    ...
    function renderPlanCanvas(selectedCenter, selectedRing, printScale) {
        var canvasW=900, canvasH=800;
        var groundW=0.180*printScale, groundH=0.160*printScale;
        var viewBounds=geographicWindow(selectedCenter,groundW,groundH).pad(0.03);
        var canvas=document.createElement('canvas'); canvas.width=canvasW; canvas.height=canvasH;
        var ctx=canvas.getContext('2d');
        ctx.fillStyle='#ffffff'; ctx.fillRect(0,0,canvasW,canvasH); ctx.lineJoin='round'; ctx.lineCap='round';
        if(typeof layer_AUACAD_4!=='undefined'&&layer_AUACAD_4.eachLayer){
            layer_AUACAD_4.eachLayer(function(parcelLayer){
                ...
                try{
                    ...
                    collectRings(parcelLayer.getLatLngs(),[]).forEach(function(ring){drawRing(ctx,ring,selectedCenter,groundW,groundH,canvasW,canvasH,'#707070',1.35);});
                }catch(e){}
            });
        }
        if(selectedRing)drawRing(ctx,selectedRing,selectedCenter,groundW,groundH,canvasW,canvasH,'#e00000',4.5);
        return canvas;
    }
    ...
    async function exportFicheParcellaireV3(sourceLayer) {
        ...
        try{
            ...
            doc.setTextColor(25,25,25); doc.setFont('helvetica','bold'); doc.setFontSize(17);
            ...
            [['Lot',p.lot],['Nature',p.nature],['Cercle',p.cercle],['Localité',p.localite],['Surface (m²)',p['Aire m²']],['TF Global',p['TF Global']],['Échelle','1:'+printScale]].forEach(function(row){
                ...
                doc.setFont('helvetica','normal'); doc.text(safeValue(row[1]),48,y); y+=7;
            });

            var planX=15,planY=82,frameW=180,frameH=160;
            doc.addImage(imgData,'PNG',planX,planY,frameW,frameH);
            // Cadre noir distinct du parcellaire gris : 0,6 mm, comme validé.
            ...
            doc.text('N',nx,ny-4,{align:'center'}); doc.setFillColor(120,120,120);
            doc.triangle(nx,ny-2,nx-2.6,ny+6,nx+2.6,ny+6,'F');

            var lx=planX+6,ly=planY+frameH-9;
            doc.setFillColor(255,255,255); doc.setDrawColor(190,190,190); doc.setLineWidth(0.2);
            doc.rect(lx-2.5,ly-4.5,23,8.5,'FD');
            doc.setDrawColor(224,0,0); doc.setLineWidth(0.8); doc.rect(lx,ly-2.1,6,4.2);
            doc.setTextColor(90,90,90); doc.setFont('helvetica','normal'); doc.setFontSize(8);
            doc.text(safeValue(p.lot),lx+8.5,ly+0.9);

            doc.setDrawColor(45,105,155); doc.setLineWidth(0.35); doc.line(15,274,195,274);
